...
public class BookLentDao {
    ...
    public List<BookLent> getBookLentNotReturned (Long clientId, Long bookId) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            query.select(root).where(criteriaBuilder.equal(root.get("client"), clientId), criteriaBuilder.equal(root.get("book"), bookId), criteriaBuilder.isNull(root.get("dateReturned")));

            bookLents.addAll(session.createQuery(query).getResultList());
        }
        return bookLents;
    }
}
